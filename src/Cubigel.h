/*
This program defines the Cubigel class header. The Cubigel compressors that
support the FDC1 protocol can be accessed using this library. The detailed
protocol description can be downloaded as a PDF file from
https://www.sv-zanshin.com/r/manuals/cubigel_fdc1_communication_protocol.pdf

The most recent version of the library is at
https://github.com/SV-Zanshin/Cubigel/archive/master.zip and the library wiki is
to be found https://github.com/SV-Zanshin/Cubigel/wiki.

This library uses the Arduino SoftwareSerial library, and it is important to
only use those pins which support pin change interrupts for the fridge and
freezer inputs. A good description of the pins and their uses can be found at
https://www.arduino.cc/en/Reference/AttachInterrupt.

While the Cubigel transmits data every 0.5 seconds, this library is designed to
use its own interrupt to read the data and to compile statistics until such time
as the calling program is ready to consume it. The Arduino SoftwareSerial
library is used, but as it only supports a single port at a time it will not
work if more than one device uses the library. This library supports the use of
both hardware and software serial ports, and if more than one device is used
then at most one port can use the SoftwareSerial library/class. The pointer to
the appropriate class instance is passed in to this library during class
instantiation and the constructor is overloaded to reflect the various options
available.

Although programming for the Arduino and in c/c++ is new to me, I'm a
professional programmer and have learned, over the years, that it is much easier
to ignore superfluous comments than it is to decipher non-existent ones; so both
my comments and variable names tend to be verbose. There are several parts of
code which can be somewhat optimized, but in order to make the c++ code more
under- standable by non-programmers some performance has been sacrificed for
...
====== ========== ==========
======================================================================= 1.0.3
2020-09-30 SV-Zanshin Issue #2 - convert sources to clang-format compatibility
1.0.2  2017-08-21 SV-Zanshin Removed extraneous code, changed comments
1.0.1  2017-07-31 SV-Zanshin Prototypes contain optional parameter definitions,
the functions no longer have them declared as non-Windows compilers fail when
they do 1.0.0  2017-04-19 SV-Zanshin Cleaned up debugging code and ready for
first release 1.0.b1 2017-04-18 SV-Zanshin Added hardware serial in addition to
software serial 1.0.b2 2017-02-21 SV-Zanshin Initial coding
*/
...
class CubigelClass {
 public:                                  // Publicly visible class members
  ...
  CubigelClass(HardwareSerial *serial1,
               SoftwareSerial *serial2);  // 2 devices - HW then SW
  CubigelClass(SoftwareSerial *serial1,
               HardwareSerial *serial2);  // 2 devices - SW then HW
  CubigelClass(HardwareSerial *serial1,
               HardwareSerial *serial2);  // 2 devices - HW and HW
  uint16_t    readValues(const uint8_t idx, uint16_t &RPM,
                         uint16_t & mA,  // Just return RPM and mA
  ...
  void        readSettings(const uint8_t idx,
                           ...
                           uint16_t &in24V, uint16_t &out42V, uint16_t &in42, uint8_t &mode);
  void        requestSettings(const uint8_t idx);  // Request a settings measurement
  bool        readTiming(const uint8_t idx, uint32_t &onTime,
                         uint32_t &offTime);              //  changes
