...
any subsequent changes by another program will not get picked up. The most recent version of the
library as well as of this program can be downloaded from
https://github.com/SV-Zanshin/Cubigel/archive/master.zip and the library wiki is to be found at
https://github.com/SV-Zanshin/Cubigel/wiki.
...
This means that if two devices (or potentially more) devices are to be monitored then all but one
must use the Arduino's UARTs. This, in turn, means that only such Atmel chips as the ATMega, which
has 4 hardware UARTs, can be used if one wishes to monitor more than 2 devices.
...
void setup() {             // Start One-Time run section
  ...
  Serial.println(F("Cubigel example program [v1.0.3]"));
