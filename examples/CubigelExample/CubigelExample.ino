/*******************************************************************************
This program is a simple example for the Cubigel compressor communications
library class. The class, once instantiated, will automatically collect
statistics in the background using an interrupt so that any program using the
library can do any processing it needs until such time as it requests the most
recent statistics. The system settings are only collected once when the class is
instantiated, any subsequent changes by another program will not get picked up.
The most recent version of the library as well as of this program can be
downloaded from https://github.com/SV-Zanshin/Cubigel/archive/master.zip and the
library wiki is to be found at https://github.com/SV-Zanshin/Cubigel/wiki.

The "SoftwareSerial library" (see
https://www.arduino.cc/en/Reference/SoftwareSerial) can be used for one device,
but not for more. This is because the library only looks for pin change
interrupts on one port at a time, and since this library is designed to work in
the background using it's own interrupt to get triggered it cannot switch the
SoftwareSerial active port around at the same time. This means that if two
devices (or potentially more) devices are to be monitored then all but one must
use the Arduino's UARTs. This, in turn, means that only such Atmel chips as the
ATMega, which has 4 hardware UARTs, can be used if one wishes to monitor more
than 2 devices.
...
SoftwareSerial FridgeSerial(FRIDGE_RX_PIN,
                            FRIDGE_TX_PIN); // Instantiate Fridge serial port
// Instantiate with one software serial port and one hardware serial port
...
void setup() {            // Start One-Time run section
  ...
  Serial.println(F("___________________________________________________________"
                   "____________________"));
  Serial.println(F("|        | Compressor |      12V      |      24V      |    "
                   "  42V      | Run   |"));
  Serial.println(F("| Device |  min/max   |  Out  /  In   |  Out  /  In   |  "
                   "Out  /  In   | Mode  |"));
  Serial.println(F("|========|============|===============|===============|===="
                   "===========|=======|"));
  for (uint8_t idx = 0; idx < LastElement; idx++) { // Loop for every device
    ...
    if (idx == Fridge) // Display appropriate device name
  ...
  Serial.println(F("|________|____________|_______________|_______________|____"
                   "___________|_______|"));
...
void loop() {                            // Main program infinite loop
  ...
  if (millis() > nextInterval) {         // If it is time to display values
    uint16_t readings, RPM, mA, CommsErrors,
        CubigelError; // Declare temporary variables
    for (uint8_t idx = 0; idx < LastElement;
         idx++) { // Loop for every device enumerated
      readings = Cubigel.readValues(idx, RPM, mA, CommsErrors,
                                    CubigelError); // Read and reset values
      uint16_t cycleSecs = 0;                      // store the cycle time
      if (Cubigel.readTiming(idx, tempStartTime,
                             tempStopTime)) { // Read the Start/Stop time
      ...
      if (readings) { // If anything has been read, show
        ...
        sprintf(sprintfBuffer, "%4d %4d %5d ", readings, RPM,
                mA);                 // Format output string
